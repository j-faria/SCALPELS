...
def ccf2acf(ccf):
    """ Compute the ACF of each CCF in the time series (eq. 3 in [1])
    ...
    acf = np.zeros_like(ccf)
    for i in range(Nrv):
        ccf_shifted = np.roll(ccf_norm, i, axis=1)
        acf[:, i] = np.sum(ccf_norm * ccf_shifted, axis=1) / acf0

    # re-normalize the ACF
    acf /= acf.mean(axis=1, keepdims=True)
